...
public class MenuApprovalAction extends Action {
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			System.out.println("MenuApprovalAction:" + frm.getButton());
			...
			} else if (commonSession.getUserCom() == null || commonSession.getUserCom().length() == 0 || !commonSession.getUserCom().equals(Param.USER_APPROVAL)) {
				throw new AuthException();
			...
			} else if (frm.getButton().equals("approval")) {
				ListApprovalActionForm f = doApproval(dbConn, commonSession);
	 ...
	 * @return
	 */
	private ListApprovalActionForm doApproval(Connection dbConn, CommonSession commonSession) throws Exception {
