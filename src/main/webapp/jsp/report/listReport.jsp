...
<bean:define id="title"><bean:message key="title.report.list" /></bean:define><!-- 日報一覧 -->
...
<!--
	function doReportEdit(userId, viewDate, seq) {
...
<body>
  <div class="container">
    ...
    <h3 class="page-header"><bean:message key="title.report.list" /></h3><!-- 日報一覧 -->

    <html:form action="/ListReport" method="post">
      ...
      <table class="table table-hover">
        <thead>
          <tr>
            <th class="col-sm-2"><bean:message key="editSchedule.name" /></th><!-- 社員名 -->
            <th class="col-sm-2"><bean:message key="item.date" /></th><!-- 日付 -->
            <th class="col-sm-2"><bean:message key="editReport.time" /></th><!-- 対応時刻 -->
            <th class="col-sm-4"><bean:message key="editReport.title" /></th><!-- タイトル -->
            <th class="col-sm-2"><bean:message key="editReport.custName" /></th><!-- 顧客名 -->
          </tr>
        </thead>
        <tbody>
          <logic:iterate id="item" property="reportList" name="ListReportForm">
            <tr>
              <td><bean:write property="userName" name="item" /></td>
              ...
              <% String itemStr = "javascript:doReportEdit('" + userId + "','" + viewDate + "','" + seq + "')"; %>
              <td><html:link href="<%=itemStr%>">
                <bean:write property="startTime" name="item" /><bean:message key="item.fromTo" /><bean:write property="endTime" name="item" />
              </html:link></td>
              <td><bean:write property="title" name="item" /></td>
              <td><bean:write property="custName" name="item" /></td>
            </tr>
          </logic:iterate>
        </tbody>
      </table>
